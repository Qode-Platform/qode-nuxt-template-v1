 ...
 *
 * HOW TO FIX AN OFFENDER (Nuxt)
 * --------------------------------------
 * Nuxt prefixes `<NuxtLink>`, `~/assets` imports and its own bundle/public
 * asset URLs with `app.baseURL` (set from BASE_PATH in nuxt.config.ts).
 * Raw string URLs are NOT touched.
 *
 * Use useRuntimeConfig().app.baseURL (note: it already ends with "/"):
 *
 *     const { app } = useRuntimeConfig();
 *     const data = await $fetch(`${app.baseURL}api/items`);
 *     <img :src="`${app.baseURL}logo.svg`" />
 *
 * EXEMPTING A LINE
 * ----------------
